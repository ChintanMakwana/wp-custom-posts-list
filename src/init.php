...
function cm_custom_posts_list_render_callback( $attributes ){
	...
	if( count( $posts ) ){
		...
		echo '<div class="cm-posts-list-wrapper '. $block_classes_attr .'" id="cm-posts-list-wrapper" '.$block_style_attr.'>';
		foreach( $posts as $post ){
			
			echo '<div class="'. implode( ' ', get_post_class('cm-posts-list-item') ) .'" id="post-'. get_the_ID() .'">';

			echo '<p><label>'. $post->post_title .'</label></p>';			
			if( $attributes['showCategoryList'] ){
				echo '<p>'. get_the_category_list(', ', '', $post->ID) . '</p>';
			}
			if( $attributes['showImage'] && has_post_thumbnail( $post->ID ) ){
				echo '<p>'. get_the_post_thumbnail($post->ID, 'post-thumbnail') . '</p>';
			}
